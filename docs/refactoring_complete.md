...
**ファイル:** `pack_for_review.ps1`, `pack_for_review2.ps1`
...
**差分:**
```powershell
# pack_for_review.ps1 (行23-48)
- $IncludeDirs = @(..., "config.example.yaml", ...)
+ $IncludeDirs = @(..., "config", ...)
+ $IncludeFiles = @(..., "config.example.yaml", ...)
...
**ファイル:** `src/core/naming.py`, `tests/test_naming.py` (新規作成)

**修正内容:**
- `Naming.generate_filename()` でテンプレート文字列を使用
- 欠けているキーがあっても例外にならない（安全なデフォルト/空文字）
- 既存ロジック（カテゴリ等の組み立て）をテンプレ生成のための context として活用
- `sanitize_filename` と拡張子処理は維持
...
**ファイル:** `src/core/filter.py`, `src/scheduler/scheduler.py`, `requirements.txt`

**修正内容:**
- `date_range` を実装: `FileInfo.metadata` から日付を参照してフィルタ可能にする
- 日付が取れない場合はフィルタ不一致（False）を返す
...
+           break
+   if not file_date:
+       return False  # 日付が取得できない場合はフィルタ不一致

# src/scheduler/scheduler.py (行67-71, 106-133)
...
**差分:**
```python
# src/core/downloader.py (行188-195, 275-287, 318-332)
- @retry(...)
...
```
docs: READMEと設定項目の説明を更新

- naming_rule, date_range, custom cron の実装状況を明記
- 使用される/使用されていない設定項目を整理
- 最近の改善点（v0.3.0）を追加
```
